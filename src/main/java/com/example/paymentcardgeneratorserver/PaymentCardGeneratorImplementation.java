...
public class PaymentCardGeneratorImplementation implements PaymentCardGenerator {
    ...
    public Map<CardType, List<String>> generateMapByCardTypes(final int howManyOfEach, final CardType... cardTypes) {
        ...
        if (cardTypes == null || cardTypes.length < 1)
            throw new IllegalArgumentException("Card types is null or empty");
    ...
    public Map<Long, List<String>> generateByPrefix(final int howManyOfEachPrefix, final List<Integer> lengths, final Set<Long> prefixes) {
        ...
        for (final Integer length : lengths) {
            ...
            for (final Long prefix : prefixes) {
                if (prefix.toString().length() > length)
                    throw new IllegalArgumentException(String.format("Prefix (%s) is longer than length (%d)", prefix.toString(), length));
                if (prefix < 1)
                    throw new IllegalArgumentException(String.format("Prefix (%s):  prefixes must be positive numbers", prefix.toString()));
            }
        }
